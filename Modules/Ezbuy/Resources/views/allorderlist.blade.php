									...
									<div class="grid-inner row">
										...
										<div class="col-lg-auto col-md-4 mt-4 mt-lg-0 text-start text-md-center">
											<div class="hotel-price">
												<a data-href="{{ $list->paymentlink }}" class="newtab"><b>MYR</b> {{ ($list->sellprice * config('app.rate')) + $list->shippingfee + $list->servicefee }}</a>
											</div>
											<small><em>Total Price</em></small><br>
											@if(in_array($list->status,['3','7','8']))
												<form method="POST" action="{{ route('updatestatus') }}">
												@csrf
												<input type="hidden" name="buyid" value="{{ $list->id }}">
												<button class="button button-rounded mt-4 mx-0 bg-warning"><i class="icon-line-arrow-up-circle"></i>UP</button>
												</form>
											@endif
											@if(in_array($list->status,['12']))
